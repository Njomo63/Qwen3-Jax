...
class CombinedSampling:
  ...
  def _apply_top_k(self, logits): 
    ...
    return jnp.where(logits < kth_value, -jnp.inf, logits)  
  
  def _apply_top_p(self, logits):
    probs = jax.nn.softmax(logits)
    sorted_probs, sorted_indices = jax.lax.top_k(probs, probs.shape[0])

    cumsum = jnp.cumsum(sorted_probs)
    mask = cumsum - sorted_probs > self.top_p

    keep_mask = jnp.zeros_like(logits, dtype=bool)
    keep_mask = keep_mask.at[sorted_indices].set(~mask)

    return jnp.where(keep_mask, logits, -jnp.inf)
